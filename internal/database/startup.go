...
func(d *DB) ConfigureDB() error {
	...
	if dbRes == false {
		sqlQuery = "CREATE TABLE foreign_posts (ID INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, content TEXT, date_created TIMESTAMP, title TEXT, tags TEXT, likes INT, origin_server_adress TEXT)"
		_, err = d.pool.Exec(context.Background(), sqlQuery)
		if err != nil {
			return err
		}
